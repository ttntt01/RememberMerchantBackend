...
class UserMerchantHandler {
  ...
  async getStaff(req, res) {
    ...
    let query = db.collection(
      "userMerchants/getStaff/merchants"
    );
    const staffID = req.body.id || req.params.id;
    {
        try {
            console.log('staffID ',staffID);
            let query = db.collection('users').doc(staffID);;
            let response = {};
            await query.get().then(staff => {
                if (staff.exists) {                       
                    response= {
                      staff: staff.data()
                            };
                    ...
                    throw new AppError("No staff found!");
                }
                // for (let doc of docs) {
                //     const selectedItem = {
                //         id: doc.id,
